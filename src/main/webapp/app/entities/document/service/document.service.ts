...
type RestOf<T extends IDocument | NewDocument> = Omit<T, 'issueDate' | 'expiryDate'> & {
...
export class DocumentService {
  ...
  protected convertDateFromClient<T extends IDocument | NewDocument | PartialUpdateDocument>(document: T): RestOf<T> {
    return {
      ...document,
      issueDate: document.issueDate?.toJSON() ?? null,
      expiryDate: document.expiryDate?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restDocument: RestDocument): IDocument {
    return {
      ...restDocument,
      issueDate: restDocument.issueDate ? dayjs(restDocument.issueDate) : undefined,
      expiryDate: restDocument.expiryDate ? dayjs(restDocument.expiryDate) : undefined,
    };
  }
